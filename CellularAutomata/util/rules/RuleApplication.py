...
class RuleApplication:
    """
    Class to apply a rule to the grid of a cellular automaton.
    """
    @staticmethod
        ...
        for i in range(automaton.rows):
            for j in range(automaton.cols):
                ...
                # Rule 4: Dead cells with 3 alive neighbors becomes alive
                    ...
                    new_cells[i][j] = 1
                    continue
                new_cells[i][j] = automaton.grid[i][j]
        return new_cells
        ...
        for i in range(automaton.cols):
            for j in range(automaton.rows):
                if (j == 0 and y_off < 0) or\
                    ...
                    new_cells[(j + y_off) % automaton.rows][i] = 0 if not carry_over else automaton.grid[j][i]
                    continue
                if (i == 0 and x_off < 0) or\
                        (i == automaton.cols - 1 and x_off > 0)\
                        and not carry_over:
                    new_cells[j][(i + x_off) % automaton.cols] = 0 if not carry_over else automaton.grid[j][i]
                    continue
                new_cells[(j + y_off) % automaton.rows][(i + x_off) % automaton.cols] = automaton.grid[j][i]
        return new_cells
    ...
    def apply_pulsate_rule(automaton):
        ...
        for i in range(automaton.rows):
            for j in range(automaton.cols):
                new_cells[i][j] = abs(automaton.grid[i][j] - 1)
        return new_cells
    ...
    def apply_majority_rule(automaton, majority):
        ...
        for i in range(automaton.rows):
            for j in range(automaton.cols):
                live_neighbors = automaton.get_live_neighbors(i, j)
                new_cells[i][j] = 1 if live_neighbors + automaton.grid[i][j] >= majority else 0
        return new_cells
